<!doctype html> <html lang="en">
  <head>
	...
	.databaseInfoLeftColumn { float: left; }
	.databaseInfoRightColumn { float: right; }
...
<body>
	...
	<div clas="container">
		...
		<div id="navbar" class="navbar-spy navbar-collapse collapse" >
			<ul class="nav navbar-nav">
				<li><a href="https://github.com/ClishamJ/clisham.cricket" class="social-button clishamGithubButton" href="https://github.com/ClishamJ/clisham.cricket"><i class="fa fa-github"></i><span class="sr-only">Github</span></a></li>
			</ul>
    ...
    <div class="component-container container">
		...
		<p>Much thanks to Giuseppe Maxia and Patrick Crews for <a href="https://github.com/datacharmer/test_db">making this database available</a> as well as Fusheng Wang and Carlo Zaniolo (both at
Siemens Corporate Research) for creating it in the first place.</p>
	...
	<div class="component-container container">
		...
		<br />
		<h4 style="font-weight:700">Example 1:</h4><h4>SELECT *</h4><h4>FROM employees</h4><h4>LEFT JOIN current_dept_emp</h4><h4>ON employees.emp_no = current_dept_emp.emp_no</h4><h4>WHERE employees.emp_no = 499456;</h4>
		<div class="sample-container">
			<?php
			...
			$combinedData2 = db_select("SELECT * FROM employees LEFT JOIN salaries ON employees.emp_no = salaries.emp_no WHERE salaries.salary > '100000' AND salaries.salary < '100100' AND gender = 'F' AND employees.first_name = 'Padma'");
			$averageSalaryByDepartment = db_select("SELECT AVG(salary) AverageSalary, dept_name Department FROM salaries LEFT JOIN dept_emp ON salaries.emp_no = dept_emp.emp_no LEFT JOIN departments ON departments.dept_no = dept_emp.dept_no GROUP BY departments.dept_name;")
			?>
		...
		<h4 style="font-weight:700">Example 2:</h4><h4>SELECT *</h4><h4>FROM employees</h4><h4>LEFT JOIN salaries</h4><h4>ON employees.emp_no = salaries.emp_no</h4><h4>WHERE salaries.salary > 100000</h4><h4>AND salaries.salary < 100100</h4><h4>AND gender = 'F'</h4><h4>AND employees.first_name = 'Padma';</h4>
		<p><span style="font-weight:700">Raw Array: </span><?php print_r($combinedData2); ?></p>
		
		<br />
		<h2>Grouping</h2>
		<br />
		<p>An employee's salary, department number, and department name are all on seperate tables. Therefore, to find the average salary of each department, you must join 3 different tables (salaries, dept_emp, and departments).</p>
		<h4 style="font-weight:700">Example 1:</h4><h4>SELECT AVG(salary) AverageSalary, dept_name Department </h4><h4>FROM salaries </h4><h4>LEFT JOIN dept_emp</h4><h4> ON salaries.emp_no = dept_emp.emp_no </h4><h4>LEFT JOIN departments</h4><h4> ON departments.dept_no = dept_emp.dept_no</h4><h4>GROUP BY departments.dept_name;</h4>
		<p><span style="font-weight:700">Raw Array: </span><?php print_r($averageSalaryByDepartment); ?></p>
		
	</div>
	
	
	<!-- Wirting MySQL Functions Section  -->
	...
	<div class="component-container container">
		...
		<p>Given that department names are kept in a seperate table ('dept_emp') from the rest of the Employee information (on the 'employees' table) and are organized by the primary keys of employee number ('emp_no') and department number ('dept_no'), this function finds the string of Department Name given an Employee's ID int:</p>
		<h4>DELIMITER $$;</h4>
	...
	<!-- Next: PHP-generated HTML elements with queries/database info? Tables?  -->
